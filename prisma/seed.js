    ...
    data: [
      {
        ...
        title: "서비스 이용약관",
        content: `제1조 (목적)
본 약관은 '모두와 AAC'(이하 '서비스')가 제공하는 AI 기반 의사소통 보조 서비스 및 관련 제반 서비스의 이용과 관련하여, 서비스와 회원 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.

제2조 (용어의 정의)
1. 서비스: 회사가 구현한 AI 문장 추천, 낱말 카드 시스템, TTS 출력 등 의사소통 보조 소프트웨어를 의미합니다.
2. 회원: 본 약관에 동의하고 서비스를 이용하는 모든 사용자를 의미합니다.
3. 낱말 카드: 의사소통을 지원하는 단위로 쓰이는 정형화된 이미지와 텍스트 조합을 의미합니다.
4. 학습 데이터: 사용자가 선택하거나 수정한 문장 정보를 통해 AI 모델의 정확도를 높이는 데 사용되는 비식별 데이터를 의미합니다.

제3조 (서비스의 제공 및 변경)
1. 서비스는 다음의 기능을 제공합니다.
   • 낱말 카드로 조합한 문장 조합 및 TTS 출력
   • AI 기반 실시간 문장 추천 (AI-01, AI-05 등)
   • 개인 맞춤형 결과 통합 관리
   • 이전 대화 문장(서버 주고받기 등) 연동 지원
2. 서비스는 기술적 사양의 변경이나 AI 모델 고도화를 위해 서비스 내용을 변경할 수 있으며, 이 경우 변경 공지사항을 통해 사전 고지합니다.

제4조 (AI 문장 생성에 관한 특칙)
1. 생성 결과에 대해: AI가 생성한 추천 문장은 모델의 확률적 결과물이며, 사용자는 재생 전 해당 문장이 자신의 의도와 일치하는지 확인해야 합니다.
2. 서비스는 AI 생성 문장으로 인해 발생할 수 있는 오해나 사회적 책임은 직접 책임을 지지 않습니다.
3. 데이터 활용: 사용자가 선택한 문장을 학습에 사용하거나 선택한 문장 데이터를 수집할 수 있습니다. 단, 모든 데이터는 특정 개인을 식별할 수 없는 방법으로 비식별화하여 처리됩니다.

제5조 (사용자의 의무)
1. 회원은 다음 행위를 해서는 안 됩니다.
   • 타인의 계정을 도용하거나 부정하게 사용하는 행위
   • 서비스의 AI 모델을 역설계(Reverse Engineering)하거나 부정하게 추출하려는 행위
   • 차별·비하, 폭력, 비윤리적 내용이 담긴 낱말 카드를 공유하는 행위
2. 회원은 자신의 상태에 조건에 맞는 언어 문장(선택 주고받기 등) 생성하길 안전 수칙에 준수해야 합니다.

제6조 (유료 서비스 및 환불)
1. 서비스는 일부 고급 기능(AI 무제한 추천, 데이터 클라우드 백업 등)을 유료로 제공할 수 있습니다.
2. 결제 및 환불은 각 앱 스토어(Google Play Store 등)의 정책 및 관련 법령을 따릅니다.

제7조 (서비스 중단 및 변경)
1. 서비스는 전자거래, 서버 점검, 네트워크 장애 등 불가피한 사유가 발생한 경우 서비스 제공을 일시적으로 중단할 수 있습니다.
2. 네트워크 의존성: AI 문장 추천 기능은 인터넷 연결이 필수적이며, 네트워크 환경에 따른 서비스 지연이나 중단에 대해 서비스는 책임을 지지 않습니다.
...
2. 서비스 제공 및 맞춤화: AI 문장 추천 알고리즘 고도화, 개인별 투타 관리, TTS 목소리 설정 보관.
3. AI 모델 학습 및 개선: 사용자가 선택/수정한 문장의 비식별 처리를 통한 모델 생성 알고리즘의 품질 향상.

제2조 (처리하는 개인정보의 항목)
서비스는 이용자로부터 다음과 같은 개인정보를 수집합니다.
1. 필수 항목: 이메일 주소, 비밀번호, 닉네임, 로그 데이터, 기기 정보(OS 버전, 기기 모델명).
2. 선택 항목: 프로필 사진, TTS 목소리 선호도, 외부 입력 장치(선택 주고받기 등) 설정값.
...
3. 탈퇴한 회원의 데이터 중 비식별화된 문장 학습 데이터는 AI 모델의 일반 성능 개선 목적으로 활용될 수 있습니다.
...
서비스는 원활한 서비스 제공을 위해 다음과 같이 개인정보 처리 업무를 위탁하고 있습니다.
   ...
   • OpenAI (API): 문장 생성 모델 전송 (단, 식별 가능한 사용자 식별 정보는 모두 제거함)

제6조 (민감정보의 처리 제한)
1. 서비스는 사용자의 건강 상태나 장애 등급 등 민감정보를 직접적으로 수집하지 않습니다.
2. 다만, AAC 서비스의 특성상 대화 내용에 간접 관련 내용이 포함될 수 있으므로, 이는 전적으로 사용자의 의사에 따라 입력되는 데이터로 간주하여 철저히 암호화 관리합니다.
