...
금지: 다른 단어 추가"""
            ...
            # 단어 2개 이상: 간결 버전
            system_prompt = """AAC 사용자용 AI. 입력 단어로 문장 3개 생성. 가장 적절한 순서대로 배열.

규칙:
1. 입력 단어 100% 사용
2. 조사 필수
3. 단어 나열 금지
4. 자연스러운 길이로 표현 (필요시 여러 문장 가능)
5. 입력 없는 단어 추가 금지"""
...
JSON: {{"predictions": ["문장1", "문장2", "문장3"]}}"""
            ...
            user_prompt += f"""
CRITICAL: 각 표현마다 {len(words)}개 단어를 **전부** 사용!

입력 단어: {words_text}

1번 표현 = {words_text} 전부 사용
2번 표현 = {words_text} 전부 사용
3번 표현 = {words_text} 전부 사용

각 표현은 **독립적**이며, 서로 다른 의미/의도여야 함.
단어 분산 금지! 각 표현이 모든 단어 포함해야 함!

나쁜 예 (단어 분산):
1번: "오늘 아침 일찍 일어났어요" ❌ (일부만 사용)
2번: "밥 먹고 가방 챙겼어요" ❌ (나머지만 사용)

좋은 예 (각각 전부 사용):
1번: "오늘 아침 일찍 일어나서 씻고 밥 먹고 가방 챙겨서 나갔어요" ✅
2번: "오늘 아침에 일찍 일어났어요. 씻고 밥을 먹은 후 가방을 챙기고 나갔어요" ✅
3번: "일찍 일어나서 오늘 아침에 씻었어요. 밥 먹고 가방 챙겨서 나갔어요" ✅

규칙:
- 조사 필수
- 지시대명사: "저것"+명사 → "저 명사"
- 단어 많을 때: 자연스럽게 여러 문장으로 구성
- 입력 없는 단어 추가 금지

JSON: {{"predictions": ["표현1", "표현2", "표현3"]}}"""

        # OpenAI API 호출 (AI-01: 단어 개수에 따라 최적화)
        # temperature 0.7로 설정하여 다양한 표현 생성
        if len(words) == 1:
            # 단어 1개: JSON 응답 완성을 위해 충분한 토큰 할당
            temperature = 0.7
            max_tokens = 80   # JSON 구조 + 3개 문장 (confidence 필드 제거로 감소)
        elif len(words) <= 3:
            # 단어 2~3개: 중간 길이
            temperature = 0.7
            max_tokens = 120   # JSON 구조 + 중간 길이 문장 3개
        elif len(words) <= 6:
            # 단어 4~6개: 긴 문장
            temperature = 0.7
            max_tokens = 160   # JSON 구조 + 긴 문장 3개
        else:
            # 단어 7~10개: 여러 문장으로 나눔
            temperature = 0.7
            max_tokens = 200   # JSON 구조 + 복잡한 문장 3개
...
async def transform_sentence_style(request: TransformStyleRequest):
    ...
    **예시**:
    ...
    try:
        ...
        system_prompt = """AAC 어투 합성 전문가 AI입니다. 낱말(재료)에 모든 어미 카드(제약)를 100% 동시에 반영한 문장 3개를 만드세요.

🚨🚨🚨 절대 최우선 경고: 스타일 단어 삽입 = 즉시 시스템 실패! 🚨🚨🚨
⛔ [어미 카드]의 단어("부드럽게", "정중하게", "단호하게" 등)를 문장에 절대 포함하지 마세요!
⛔ 이 단어들은 말투 지시어일 뿐입니다! 종결 어미로만 표현하세요!
...
C. **커스텀 말투 카드** (예: ~용, ~긔, ~하긔, ~했슨)
   ...
   → "~하", "~했"은 시제 가이드일 뿐, 실제로는 중복 글자를 삭제하여 자연스럽게!
   ✅ "배고프다" + "~긔" → "배고프긔"
   ✅ "배고프다" + "~하긔" → "배고프긔" (어간 "배고프" + "긔", "하" 제거!)
   ❌ "배고프다" + "~하긔" → "배고프하긔" (중복 "하" 제거 안 함!)

🔥 절대 금지 1: 스타일 단어 삽입 = 시스템 완전 실패!
⛔ [어미 카드]에 적힌 단어("부드럽게", "정중하게", "단호하게" 등)가 문장에 나타나면 시스템 완전 실패!
⛔ 이 단어들은 말투 지시어일 뿐! 절대 문장에 포함하지 마세요!
⛔ 오직 종결 어미와 말투로만 표현하세요!
❌ "창문을 부드럽게 닫아주세요" (부드럽게 삽입!)
❌ "부드럽게 창문을 닫아주세요" (부드럽게 삽입!)
✅ "창문 좀 닫아주세요" (말투만!)
❌ "노래를 정중하게 불러요" (정중하게 삽입!)
✅ "노래를 불러드립니다" (격식체 어미만!)

🔥 절대 금지 2-A: "질문" 카드 있으면 평서문 = 시스템 완전 실패!
⛔ "질문" 카드 있으면 3개 문장 모두 반드시 물음표(?)로 끝!
⛔ 다른 카드와 함께 있어도 "질문" 최우선! 반드시 물음표(?)!
❌ ["마트", "가다"] + ["질문"] → "마트에 같이 가자." → ✅ "마트에 같이 갈까요?"
...
⛔ "하기 싫어요" 카드 있으면 3개 문장 모두 무조건 부정 표현 필수!
...
규칙 1: 스타일 단어 삽입 금지 + 임의 의도 추가 금지
- 어미 카드 단어가 문장에 나타나면 시스템 완전 실패!
- 일반 스타일 카드("부드럽게" 등)에 욕구/부정/거부 표현 임의 추가 절대 금지!

규칙 2: "질문" 카드 유무에 따른 문법 모드
- "질문" 있음 → 3개 문장 모두 물음표(?)로 끝! 마침표(.), 느낌표(!) 금지!
- "질문" 없음 → 의문형 어미(~ㄹ까요?, ~나요?, ~을까?) 절대 금지! 평서문만(~주세요, ~해요, ~습니다)!

규칙 3: "하기 싫어요"/"하고 싶어요" 의미 강제 (1인칭 주체!)
...
규칙 4: 형태소 융합 (커스텀 어미 처리)
- 커스텀 어미는 용언 어간에서 '-다'를 제거하고 직접 결합!
- 🚨 특히 "~하긔", "~했슨" 같은 카드는 중복 글자 제거 필수!
  → "~하", "~했"은 시제(현재/과거) 가이드일 뿐, 실제로는 중복 글자 삭제!
- ✅ "배고프다" + "~하긔" → "배고프긔" (어간 "배고프" + "긔", "하" 제거!)
- ❌ "배고프다" + "~하긔" → "배고프하긔" (중복 "하" 제거 안 함!)
- ✅ "예쁘다" + "~했슨" → "예뻤슨" (어간 "예쁘" + "었슨")
- ✅ "먹다" + "~용" → "먹용" (어간 "먹" + "용")

규칙 5: 모든 낱말 카드 포함 강제 + 단어 추가 절대 금지
- 각 문장마다 모든 낱말 카드를 100% 사용!
- 낱말 카드 외 명사/동사/형용사/부사 추가 절대 금지! (조사와 어미만 변형!)
- ❌ ["지금", "자다"] → "지금 졸려요" (자다 누락!) → ✅ "지금 자고 싶어요"
- ❌ ["선생님"] → "선생님, 수업 시작해요" (수업, 시작 추가!) → ✅ "선생님?" / "선생님이세요?"

규칙 6: 제로 할루시네이션 (최우선!)
- 낱말 카드 외 명사, 동사, 형용사, 부사 추가 절대 금지!
- 조사(을/를/이/가/에게)와 어미만 변형 가능!
- ❌ ["선생님"] → "선생님, 수업을 진행해도 될까요?" (수업, 진행 추가!)
- ✅ ["선생님"] → "선생님?" / "선생님이세요?" / "선생님께?"

규칙 7: 구조적 변주 + 중복 절대 금지!
- 3개 문장은 반드시 서로 달라야 함! 동일한 문장 2개 이상 절대 금지!
- 단어 추가 없이 조사 변형, 보조 용언(~긴, ~기도), 어미 변형으로 구조적 차이!
- ❌ ["배고프다"] + ["~하긔"] → "배고프긔", "배고프긔", "배고프긔" (중복!)
- ❌ ["배고프다"] + ["~하긔"] → "배고프하긔" (융합 실패!) / "배고프다 하긔" (중복!)
...
1. **"질문" 카드 확인 (시스템 생사 결정!)**: {endingCards_text}에 "질문"이 있습니까?
   → **있으면**: 3개 문장 모두 반드시 물음표(?)로 끝! 평서문/명령문/감탄문 금지!
   → **없으면**: 3개 문장 중 단 하나도 물음표(?) 금지! 의문형 어미(~ㄹ까요?, ~세요?, ~해요?, ~래요?, ~나요?) 절대 사용 금지!

   예시 확인:
   - ["안", "하다"] + ["단호하게"] → "질문" 없음 → "안 해!" / "안 한다!" (✅) / "안 하세요?" / "안 할까요?" (❌ 의문형 금지!)
   - ["밥", "먹다"] + ["질문"] → "질문" 있음 → "밥 먹을까요?" / "밥 드실래요?" (✅) / "밥 먹어." / "밥 먹어!" (❌ 평서문 금지!)
...
| 카드 유형 | 구현 방법 | ✅ 정답 | ❌ 실패 |
|:---|:---|:---|:---|
| **부드럽게** | 중립 평서문 어미만(~주세요, ~해요, ~게요) + 질문 카드 없으면 의문형 금지! | 밥 먹어요 / 창문 좀 닫아주세요 | 밥 먹을래요? (의문형!) / 밥 먹기 싫어 (임의 의도!) |
| **단호하게** | 강한 종결 어미(~해!, ~한다!) + 질문 카드 없으면 의문형 금지! | 안 해! / 안 한다! / 절대 안 해! | 안 하세요? / 안 할까요? (의문형 금지!) |
| **정중하게** | 격식체 평서문(~습니다) + 질문 카드 없으면 의문형 금지! | 노래를 부릅니다 | 노래 부르실래요? (의문형!) |
| **질문** | 반드시 물음표(?) - 3개 문장 모두! | 마트에 갈까요? / 갈래요? | 마트에 가자. (평서문!) |
| **하기 싫어요** | 부정 표현 필수 (나의 거부) | 약 안 먹어! / 약 먹기 싫어요! | 약 먹어! (긍정문!) |
| **하고 싶어요** | 긍정 욕구 표현 필수 (나의 욕구) + 부정 표현 금지 | 놀이터 가고 싶어요 / 자고 싶어! | 놀이터 가! (욕구 없음!) / 자고 싶지 않아? (부정!) |
| **~긔, ~용, ~하긔** | 어간에 직접 결합 + 중복 글자 제거! | 배고프긔 (배고프+긔) / 먹용 (먹+용) | 배고프하긔 (중복 "하"!!) |

🚨 최종 체크리스트 (출력 전 필독!)

0. **임의 의도 추가 검열**: 일반 스타일 카드("부드럽게", "정중하게")만 있는데 욕구/부정/거부 표현 추가했는가?
   → "-고 싶다", "-기 싫다", "안~", "못~" 등의 의도 표현이 있으면 즉시 재생성!
   ❌ ["밥", "먹다"] + ["부드럽게"] → "밥 먹기 싫어" / "밥 안 먹을래"
   ✅ ["밥", "먹다"] + ["부드럽게"] → "밥 먹어요" / "밥 드세요"

1. **질문 카드 검열 (최최우선!)**: {endingCards_text}에 "질문"이 있는가?
   → **있으면**: 3개 문장 모두 물음표(?)로 끝? (마침표, 느낌표 금지!)
   → **없으면**: 3개 문장 중 단 하나도 물음표(?) 없어야 함! 의문형 어미(~ㄹ까요?, ~세요?, ~래요?) 절대 금지!
   ❌ ["안", "하다"] + ["단호하게"] → "안 하세요?" / "안 할까요?" (질문 없는데 의문형!)
   ✅ ["안", "하다"] + ["단호하게"] → "안 해!" / "안 한다!"
   ❌ ["잠", "자다"] + ["질문", "하고 싶어요"] → "잠을 자고 싶어!" (질문 있는데 평서문!)
   ✅ ["잠", "자다"] + ["질문", "하고 싶어요"] → "잠 자도 될까요?"

2. **부정 표현 검열**: "하기 싫어요" 있으면 3개 문장 모두 부정 표현(안/못/-지 않다/-기 싫다) 포함?
   ❌ ["약", "먹다"] + ["하기 싫어요"] → "약 먹어!"
   ✅ ["약", "먹다"] + ["하기 싫어요"] → "약 안 먹어!"

3. **욕구 표현 검열**: "하고 싶어요" 있으면 3개 문장 모두 긍정 욕구 표현(-고 싶다/-려고 하다/-고 싶은데) 포함? + 부정 표현(-지 않다/안/못) 절대 금지! (1인칭 주체!)
   ❌ ["잠", "자다"] + ["질문", "하고 싶어요"] → "너 잠 자고 싶어?" (상대방에게 묻기!)
   ❌ ["잠", "자다"] + ["질문", "하고 싶어요"] → "잠 자고 싶지 않아?" (부정 표현!)
   ✅ ["잠", "자다"] + ["질문", "하고 싶어요"] → "잠 자고 싶어?" / "잠 자도 될까요?"

4. **스타일 단어 삽입 금지 확인 (절대 최우선!)**: "{endingCards_text}"에 있는 단어가 문장에 포함되었는가?
   → ⛔ "부드럽게", "정중하게", "단호하게", "친근하게" 등의 단어가 문장에 있으면 안 됨!
   → 포함되었다면 해당 단어를 즉시 삭제하고 말투로만 바꿔서 재생성!
   ❌ "창문을 부드럽게 닫아주세요" (부드럽게 삽입!) → ✅ "창문 좀 닫아주세요" (말투만!)
   ❌ "부드럽게 창문을 닫아주세요" (부드럽게 삽입!) → ✅ "창문 닫아주세요" (말투만!)
   ❌ "노래를 정중하게 불러요" (정중하게 삽입!) → ✅ "노래를 불러드립니다" (격식체 어미만!)

5. **형태소 융합 확인 (커스텀 어미 필수!)**: 커스텀 어미(~하긔, ~했슨, ~용, ~긔 등) 사용 시 중복 글자 제거했는가?
   → "~하긔"의 경우 "하"를 제거! "~했슨"의 경우 중복 시제 제거!
   ❌ ["배고프다"] + ["~하긔"] → "배고프하긔" (중복 "하" 제거 안 함!)
   ✅ ["배고프다"] + ["~하긔"] → "배고프긔" (어간 "배고프" + "긔")

6. **모든 낱말 카드 포함**: 각 문장이 {words_text} 전부 포함? (하나라도 누락 시 재생성!)

7. **단어 추가 금지 검열 (최우선!)**: {words_text} 외에 명사/동사/형용사/부사를 추가했는가?
   → 추가된 단어가 있으면 즉시 재생성! 조사와 어미만 변형 가능!
   ❌ ["선생님"] + ["질문"] → "선생님, 수업을 진행해도 될까요?" (수업, 진행 추가!)
   ✅ ["선생님"] + ["질문"] → "선생님?" / "선생님이세요?" / "선생님이신가요?"

8. **의문형 어미 최종 검열**: "질문" 카드 없는데 ~ㄹ까요?, ~세요?, ~해요?, ~나요?, ~을까?, ~래요? 사용했는가?
   → 의문형 어미 발견 시 즉시 재생성! 질문 카드 없으면 평서문/명령문/감탄문만!

9. **구조적 변주**: 단어 추가 없이 조사/어순/길이만으로 3개 문장이 충분히 다른가?

JSON: {{"sentences": ["문장1", "문장2", "문장3"]}}"""

        # OpenAI API 호출 (AI-05: 단어 개수에 따라 최적화)
        # temperature 0.1로 극도로 낮춰 지시 사항(스타일 단어 삽입 금지) 엄격히 준수
        # presence_penalty 1.5로 강화하여 문장 중복 방지
        if len(words) == 1:
            # 단어 1개: JSON 응답 완성을 위해 충분한 토큰 할당
            max_tokens = 180   # JSON 구조 + 어미 적용 문장 3개 (반드시 3개 생성)
        elif len(words) <= 3:
            # 단어 2~3개: 중간 길이
            max_tokens = 200   # JSON 구조 + 중간 길이 어미 적용 문장 3개
        elif len(words) <= 6:
            # 단어 4~6개: 긴 문장
            max_tokens = 250   # JSON 구조 + 긴 어미 적용 문장 3개
        else:
            # 단어 7~10개: 매우 긴 문장
            max_tokens = 300   # JSON 구조 + 복잡한 어미 적용 문장 3개
